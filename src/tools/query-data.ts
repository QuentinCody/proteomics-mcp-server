...
interface QueryEnv {
    PROTEOMICS_DATA_DO?: unknown;
}

export function registerQueryData(server: McpServer, env?: QueryEnv): void {
    const handler = createQueryDataHandler("PROTEOMICS_DATA_DO", "proteomics");

    server.registerTool(
        "proteomics_query_data",
        {
            title: "Query Staged Proteomics Data",
            description:
                "Query staged PRIDE Archive + ProteomeXchange data using SQL. Use this when responses are too large and have been staged with a data_access_id.",
            inputSchema: {
